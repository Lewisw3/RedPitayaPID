...
(
   ...
   input      [ 12-1: 0] dat_i           ,  // input data
   output     [ 12-1: 0] dat_o           ,  // output data

   // settings
   input      [ 12-1: 0] set_sp_i        ,  // set point
   input      [ 12-1: 0] set_kp_i        ,  // Kp
   input      [ 12-1: 0] set_ki_i        ,  // Ki
   input      [ 12-1: 0] set_kd_i        ,  // Kd
   input                 int_rst_i          // integrator reset
);

// the values range from 0 to 4095 and will all be positive
...
always @(posedge clk_i) begin
   if (rstn_i == 1'b0) begin
      ...
      error <= $unsigned(set_sp_i) - $unsigned(dat_i) ; //error could be a negative number - we need signed calculations
   end
end
...
always @(posedge clk_i) begin
   if (rstn_i == 1'b0) begin
      ...
      kp_reg <= kp_mult[25-1:PSR] ;
   end
end

assign kp_mult = $unsigned(error) * $unsigned(set_kp_i);
